...
2. **First unlock** — EN | VI | ES switch on every shop page (skeletons included). Chrome and action labels come from `i18n/phrases.json` via `t(key)`. Story / vibe body is never machine-translated; missing VI/ES falls back to EN for that key only.
